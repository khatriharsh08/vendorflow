...
export default function PaymentsIndex({ payments, stats, currentStatus }) {
    ...
    const statCards = [
        ...
        { label: 'Total Paid', value: `₹${(stats?.paid || 0).toLocaleString('en-IN')}`, icon: '💰', color: 'primary' },
        { label: 'Total Transactions', value: stats?.total || 0, icon: '📋', color: 'info' },
    ];

    const columns = [
        { header: 'Reference', render: (row) => <span className="font-mono text-(--color-text-primary) font-medium">{row.reference_number}</span> },
        { header: 'Vendor', render: (row) => <span className="text-(--color-text-secondary)">{row.vendor?.company_name}</span> },
        { header: 'Amount', align: 'right', render: (row) => <span className="text-(--color-text-primary) font-bold">₹{parseFloat(row.amount).toLocaleString('en-IN')}</span> },
            ...
            render: (row) => (
                <div className="flex gap-2 justify-end">
                    {row.status === 'requested' && can.validate_payments && (
                        <>
                            <Button variant="success" size="sm" onClick={() => handleAction(row.id, 'ops', 'approve')}>Validate</Button>
                            <Button variant="danger" size="sm" onClick={() => handleAction(row.id, 'ops', 'reject')}>Reject</Button>
                        </>
                    )}
                    {row.status === 'pending_finance' && can.approve_payments && (
                        <>
                            <Button variant="success" size="sm" onClick={() => handleAction(row.id, 'finance', 'approve')}>Approve</Button>
                            <Button variant="danger" size="sm" onClick={() => handleAction(row.id, 'finance', 'reject')}>Reject</Button>
                        </>
                    )}
                    {row.status === 'approved' && can.mark_paid && (
                        <Button variant="primary" size="sm" onClick={() => { setSelectedPayment(row.id); setShowMarkPaidModal(true); }}>
                            Mark Paid
                        </Button>
                    )}
                    {row.status === 'requested' && !can.validate_payments && <span className="text-xs text-(--color-text-tertiary) italic">Waiting for Ops</span>}
                    {row.status === 'pending_finance' && !can.approve_payments && <span className="text-xs text-(--color-text-tertiary) italic">Waiting for Finance</span>}
                    {row.status === 'approved' && !can.mark_paid && <span className="text-xs text-(--color-text-tertiary) italic">Ready for Payment</span>}
                </div>
            )
        },
    ];
            ...
            <Modal 
                ...
                footer={
                    <>
                        ...
                        <ModalPrimaryButton onClick={handleMarkPaid} disabled={!paymentRef}>Confirm Payment</ModalPrimaryButton>
                    </>
                }
            >
                <div className="space-y-4">
                    <div>
                        <label className="text-sm font-medium text-(--color-text-secondary) mb-2 block">Payment Reference *</label>
                        <input
                            type="text"
                            value={paymentRef}
                    ...
                    <div>
                        <label className="text-sm font-medium text-(--color-text-secondary) mb-2 block">Payment Method</label>
