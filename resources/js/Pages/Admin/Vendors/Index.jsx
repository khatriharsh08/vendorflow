...
export default function VendorsIndex({ vendors = [], currentStatus = 'all', search = '' }) {
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        router.get('/admin/vendors', { status: currentStatus, search: searchQuery }, { preserveState: true });
    };
    ...
    const columns = [
        { 
            ...
            render: (row) => (
                <div>
                    <div className="text-[var(--color-text-primary)] font-medium">{row.company_name}</div>
                    <div className="text-sm text-[var(--color-text-tertiary)]">{row.contact_email}</div>
                </div>
            )
        },
        { header: 'Contact', render: (row) => <span className="text-[var(--color-text-secondary)]">{row.contact_person}</span> },
        { header: 'Status', render: (row) => <Badge status={row.status} /> },
        { 
            header: 'Performance', 
            align: 'center',
            render: (row) => (
                <span className={`font-bold ${row.performance_score >= 70 ? 'text-[var(--color-success)]' : row.performance_score >= 40 ? 'text-[var(--color-warning)]' : 'text-[var(--color-danger)]'}`}>
    ...
    const statusFilters = ['all', 'draft', 'submitted', 'under_review', 'approved', 'active', 'suspended'];
