...
export default function Dashboard() {
    ...
    const stats = [
        { label: 'Total Vendors', value: '128', icon: '🏢', change: '+12%', color: 'from-blue-500 to-cyan-400' },
        { label: 'Active', value: '94', icon: '✅', change: '+5%', color: 'from-green-500 to-emerald-400' },
        { label: 'Pending Review', value: '23', icon: '⏳', change: '-3%', color: 'from-yellow-500 to-orange-400' },
        { label: 'Non-Compliant', value: '11', icon: '⚠️', change: '+2', color: 'from-red-500 to-rose-400' },
    ];

    const recentActivities = [
        { action: 'Vendor Approved', vendor: 'TechCorp Solutions', user: 'Admin', time: '2 mins ago', type: 'success' },
        { action: 'Payment Requested', vendor: 'Global Services Ltd', user: 'Vendor', time: '15 mins ago', type: 'info' },
        { action: 'Document Expired', vendor: 'InfoSys Partners', user: 'System', time: '1 hour ago', type: 'warning' },
        { action: 'Vendor Suspended', vendor: 'DataFlow Inc', user: 'Admin', time: '3 hours ago', type: 'danger' },
    ];
                    ...
                    <header className="sticky top-0 z-40 glass-card border-b border-slate-800/50 px-8 py-4">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-slate-400 text-sm">Welcome back, {user?.name?.split(' ')[0] || 'User'}!</p>
                            </div>
                                    ...
                                    {recentActivities.map((activity, idx) => (
                                        <div key={idx} className="flex items-center gap-4 p-3 rounded-lg hover:bg-slate-800/50 transition-colors">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="text-slate-400 text-xs truncate">{activity.vendor}</div>
                                            </div>
                                ...
                                <div className="space-y-4">
                                    {pendingApprovals.map((item, idx) => (
                                        <div key={idx} className="flex items-center justify-between p-4 rounded-lg bg-slate-800/30 border border-slate-700/50">
                                            <div>
                                                <div className="text-xs text-indigo-400 font-medium mb-1">{item.type}</div>
